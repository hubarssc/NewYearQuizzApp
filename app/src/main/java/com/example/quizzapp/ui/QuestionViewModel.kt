...
class QuestionViewModel @Inject constructor(private val questionRepository: QuestionRepository) :
    ...
    private var questionsForSession: MutableLiveData<ArrayList<Question>> =
    ...
    fun startQuizSession() {
        questionsForSession.value =
            QuestionDataSource.category?.let {
                questionRepository.getQuestionsForQuizSession(
                    it
                )
            }?.let { ArrayList(it) }
        getNextQuestion()
